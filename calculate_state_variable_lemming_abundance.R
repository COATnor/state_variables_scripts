...
lem_snap <- snap_dat %>%  filter(v_species %in% c("lem_lem")) %>% 
  group_by(sn_region, sn_locality, sn_section, sn_site, sc_type_of_sites_ecological, t_year, t_season) %>% 
  summarise(v_rodents = sum(v_abundance, na.rm = TRUE)) %>% 
  mutate(v_rodents = log(((v_rodents/24)*100)+0.1)) %>%
  mutate(t_date = ifelse(t_season == "spring", paste0(t_year, "-07-03"), paste0(t_year, "-09-03")))
